...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = useCallback(async (id: string, newRole: string) => {
    await fetch(`/api/v1/admin/users/${id}/role`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ role: newRole }) });
    mutate();
  }, [mutate]);

  const handleToggleStatus = useCallback(async (id: string, newStatus: string) => {
    await fetch(`/api/v1/admin/users/${id}/status`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: newStatus }) });
    mutate();
  }, [mutate]);

  const handleExport = useCallback(async (id: string) => {
    await fetch(`/api/v1/admin/users/${id}/export`, { method: "POST" });
  }, []);
